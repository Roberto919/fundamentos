...
```{r sims-te-2}
set.seed(2)
reps <- lineup(null_permute("Tea"), te_azucar, n = 100)
valores_ref <- reps %>% 
  group_by(.sample) %>% 
  nest() %>% 
...
# permutar
valores_ref <- permutaciones_est(fusion, nv.vv, calc_cociente, n = 1000)
...
g_2 <- ggplot(valores_ref, aes(x = diferencia)) + geom_histogram(binwidth = 0.04) + 
    coord_flip() + xlab("") + labs(subtitle = " ") +
    geom_vline(xintercept = dif_obs, colour = "red") +
    annotate("text", x = dif_obs, y = 100, label = cuantil_obs,vjust = 0, 
             colour = "red")
g_1 + g_2 
```
