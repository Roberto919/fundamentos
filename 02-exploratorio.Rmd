...
```
## Esta gráfica es una gran alternativa para mostrar mucha información estadística en una sola visualización.
```
...
ggplot(casas, aes(x = nombre_zona, y = precio_miles)) +
  ...
  coord_flip()
```
<!-- - Algo interesante que vemos es que hay más variación en las casas más costosas. (es común que pase esto en las variables positivas) -->
...
ggplot(casas, aes(x = nombre_zona, y = precio_m2)) +
  ...
  coord_flip()
```
<!-- - Los resultados de este análisis son convenientes porque los resultados se parecen. Nuestra representación es más general y nos ayuda a explicar mejor lo que pasa. -->
  <!-- - Favorecemos siempre los modelos más simples. -->
...
```
<!-- - Aquí lo que nos importa es descomponer el valor de las casas entre: -->
<!--   - Valor ajustado -->
<!--   - Residuales -->
...
## Loess {-}
<<<<<<< HEAD
<!-- - Esta técnica nos podría ayudar a ver tendencias en series de tiempo. -->
=======
>>>>>>> upstream/render-html
...
ventas <- read.csv(file="data/ventas_semanal.csv")
