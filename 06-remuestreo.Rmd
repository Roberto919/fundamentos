...
<!-- - Preferimos bootstrap sobre plug-in -->
<!--   - Vemos la distribución -->
<!--   - Podemos calcular más estadísticas (e.g. mediana, correlación) que con el plug-in -->

<!-- - Hay dos fuentes de variación -->
<!--   - Aleatorización de tomar la muestra -->
<!--   - Estoy tomando muestras con reemplazo -->
<!--     - Esta segunda se puede eliminar tomando todas las muestras bootstrap posibles -->
...
mediana es `r round(mean(casas_muestra$precio_miles), 1)`, con un intervalo de
...
entre las observaciones de la remuestra, y cómo serían las remuestras sin reemplazo.
  # - Porque tu muestra sería igualita siempre.
- ¿Por qué no se puede hacer bootstrap si no conocemos cómo se obtuvo la muestra original?
    # - Bootstrap debe replicar el proceso a través del cual se seleccionaron los datos.
    # - Debes respetar como se tomó la muestra. Replicar el proceso generador de datos.
```
...
pob_plot <- ggplot(data_frame(x = -15:20), aes(x)) +
    ...
    scale_colour_manual(values = c('mu' = 'red'), name = '',
        labels = expression(mu)) +
    labs(x = "", y = "", subtitle = expression("Población"~F), color = "") +
    theme(axis.text.y = element_blank())

samples <- tibble(sample = 1:6) %>%
    mutate(
...
emp_dists_plots <- ggplot(emp_dists, aes(x = obs)) +
    ...
    xlim(-15, 20) +
    geom_vline(xintercept = 5, color = "red", alpha = 0.5) +
    labs(x = "", y = "", subtitle = expression("Distribución empírica"~hat(F))) +
    scale_colour_manual(values = c('mu' = 'red', 'x_bar' = 'blue'), name = '',
        labels = c(expression(mu), expression(bar(x)))) +
...
(pob_plot | plot_spacer() | plot_spacer()) / (emp_dists_plots | boot_dists_30_plot | boot_dists_1000_plot) +
...
g_hist + g_qq
```
<!-- - A pesar de que la gráfica qq no se ve tan tan bien en una cola, sí la usamos porque el valor que nos interesa está en el centro. -->
...
```
<!-- - Esto es como un proceso separado... como si hiciéramos dos ejercicios independientes -->
<!-- - Esto afecta para el tema de "strata". Aquí sí tenemos que segmentar cada una por separado -->
...
<!-- - La muestra para la prueba la sacas por renglones. -->
...
```
<!-- - Un problema de este ejercicio es que hay clases en las que no tenemos intervalos. -->
<!--   - Podemos resolver agregando un perfil falso a todos los casos (para que no tengamos clases vacías). -->
<!-- - Opciones -->
<!--   - 1ra -> quitar del análisis -->
<!--   - 2da -> agrupar en otra categoría -->
<!--   - 3ra -> agregar un datos falso a cada personas. -->
...
round(mean(estudiantes$calif), 2)
```
...
```
<!-- Aquí estamos viendo que se deberían visitar 59 escuelas para sacar las 60 pruebas de examen. Claramente, este approach no es muy eficiente. -->
...
hist_cgl + qq_cgl
```
<!-- - Hay dos formas de reducir dos errores estándar -->
<!--   - Tomando muestras estratificadas  -->
<!-- - tomando muestras con base en los conglomerados -->
...
```{r, message=FALSE, warning=FALSE}
concentrado_hogar <- read_csv(here::here("Documents/ReposRob_RobPer/ITAMmcd/Fundamentos_Est/Venv_FundEst/fundamentos",
    "data",
...
hogar
```
<!-- - En este caso la columna factor se refiere al número de casas que cada renglón representa. -->
