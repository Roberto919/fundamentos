...
<!-- - El valor de máxima verosimilitud es el valor de "theta" que maximiza la verosimilitud. -->
<!-- - En este caso sería 0.6 el estimador de máxima verosimilitud. -->
<!-- - La incertidumbre la calcularíamos como un intervalo originalmente, pero aquí solo puede haber 1 solo outcome. -->
  <!-- - No podríamos crear un intervalo como tal. -->
...
probs_inicial <- tibble(moneda = c(1, 2),
                        theta = theta,
                        prob_inicial = c(0.5, 0.5)) # Primer valor es la probabilidad de águilas.
probs_inicial
# verosimilitud
...
verosim <- crear_verosim(2) # Número de éxitos (soles) que ves
...
posteriores de cada moneda?
  # - Las probabilidades se invierten. Las probabilidades posteriores.
  # - Tenemos más evidencia de que está cargada a águila.
...
```
<!-- - Recordemos que el estimador de máxima verosimilitud sería 19/30. (éxitos entre lanzamientos.) -->
...
```
<!-- - Escogimos pareto porque era un modelo adecuado. Que se parece al comportamiento de nuestra inicial. -->
