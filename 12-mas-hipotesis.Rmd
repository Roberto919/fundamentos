...
<!-- - Podemos aprovechar lo que hemos visto para pruebas de hipótesis. -->
...
<!-- - Sabemos que ciertos parámetros se comportan como normales. -->
...
```
<!-- - Nos da un valor "p" muy chico, con lo cual rechazamos la hipótesis nula. -->
<!-- - La clave es que la operación es que la ecuación entre nuestro estimador y el valor esperado se comporta como una normal estándar. -->
...
valor_p
```
<!-- - Vemos que las pruebas de Wald son fáciles de hacer. -->
...
Estado de México. ¿Hay evidencia de que tienen distintas medias?
  # - (Se cambió por hacer comparaciones de los turnos)
```
...
## Datos enlace
enlace_data <- read_csv("data/enlace_15.csv")


## Muestras
muestra <- enlace_data %>% 
  filter(turno != "NOCTURNO") %>% 
  mutate(n_muestra = ifelse(turno == "VESPERTINO", 120, 100)) %>%
  group_by(turno) %>%
  sample_n(size = first(n_muestra))
...
## Calculating "w" value
w <- (539 - 525)/sqrt(0.869^2 + 1.418^2)
w
...
<!-- - En este caso no rechazamos y podemos elegir el algoritmo rápido. -->


## Datos pareados {-}

<!-- - En este caso se trata de datos pareados porque vamos a tener dos resultados para cada obervación (uno con algoritmo 1 y otro con algoritmo 2) -->
...
<!-- - Pruebas que involucran varios parámetros. -->
...
<!-- - Con esta fórmula tomamos en cuenta la probabilidad de éxitos y fracasos. -->
...
<!-- - La "lambda" grande quiere decir que hay mucha diferencia entre lo que estamos comparando. -->
...
<!-- - ahora vamos a evaluar la "lambda" para ver si la rechazamos o no. -->
...
```
<!-- - Es la misma idea con las pruebas de permutación. -->
...
<!-- - Usando la función de pchisq podemos llegar a estos resultados cuando tenemos 1 grado de libertad. -->
<!-- - Calculamos la densidad acumulada de la distribución y restamos 1. -->
<!-- - Debemos usar pqhisq. -->
