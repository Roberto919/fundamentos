...
- ¿Qué tan buen estimador de $b/2$ es la media muestral? ¿Cómo lo cuantificarías?
  # - Este sí puede ser un buen estimador, como vimos en nuestro ejemplo arriba.
...
```
<!-- - El error estándar es la desviación de la medida de interés. Variación entre las muestras -->
...
```
<!-- - En el caso de la desviación estándar solo vamos a ver que está no cambiará, solo será más uniforme y todas tenderán al valor medio. -->
<!-- - En el caso del error estándar, este sí bajará considerablemente, porque cada vez se hará más angosa nuestra curva de estimación. -->
...
que hicimos.
<!-- - La tasa lambda es que la media es  -->
...
```
Y vemos que en efecto el TCL (Teorema Central del Límite) aplica en este ejemplo, y la aproximación es buena.
...
```{r}
## Validación de la distribución muestral
library(tidyverse)
library(patchwork)

dat_simul <- tibble(n_sim = 1:5000) %>%
  mutate(suma = map_dbl(n_sim, ~ sum(runif(30))))
...
```{r}
library(nullabor)
lineup_normal <- lineup(null_dist("suma", dist = "normal"), dat_simul)
ggplot(lineup_normal, aes(sample = suma)) +
  geom_qq(distribution = stats::qnorm) +
  geom_qq_line(colour = "red") +
  facet_wrap(~ .sample)
```
<!-- - La hipótesis nula en este caso es que la distribución es normal. -->
<!-- - Como no podemos distinguir los datos, entonces concluimos que sí son normales -->
